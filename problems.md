...
- **[Problem 21](#problem-21)**: **Dimensional Analysis in Problem Solving**
  - If pressure $P$ is related to density $\rho$ and velocity $v$ as $P = k\rho^a v^b$, find exponents $a$ and $b$ that make the equation dimensionally consistent.
...
$$
This expression is valid when $1 - \mu \cot\theta > 0$ (i.e., $\mu < \tan\theta$), ensuring motion occurs.
...
A flywheel, with moment of inertia $I$ about its axis of rotation, is initially at rest. Starting at time $t=0$, a constant torque $\tau$ is applied to the flywheel. Determine the rotational kinetic energy of the flywheel after it has completed exactly 5 full rotations.  
...
After the collision, the system rotates about the pivot with an initial angular velocity $ \omega $. The linear speed $ v $ of the combined mass at the end of the rod (distance $ L $ from the pivot) is related to the angular velocity by:
...
To find $v(T)$, integrate the acceleration function. Acceleration is the derivative of velocity:  
$$ a(t) = \frac{dv}{dt} = k t $$  
Separate variables and integrate from $t=0$ to $t$:  
$$ \int_{0}^{v(t)} dv = \int_{0}^{t} k t'  dt' $$  
$$ v(t) - 0 = k \left[ \frac{(t')^{2}}{2} \right]_{0}^{t} $$  
$$ v(t) = \frac{1}{2} k t^{2} $$  
At $t = T$:  
$$ v(T) = \frac{1}{2} k T^{2} $$  
Substitute into the average acceleration formula:  
$$ a_{\text{avg}} = \frac{\frac{1}{2} k T^{2}}{T} = \frac{1}{2} k T $$  

Now eliminate $k$ using the distance $D$. Velocity is the derivative of displacement:  
$$ v(t) = \frac{ds}{dt} = \frac{1}{2} k t^{2} $$  
Integrate from $t=0$ to $t=T$ with $s(0)=0$ and $s(T)=D$:  
$$ \int_{0}^{D} ds = \int_{0}^{T} \frac{1}{2} k t^{2}  dt $$  
$$ D = \frac{1}{2} k \left[ \frac{t^{3}}{3} \right]_{0}^{T} = \frac{1}{2} k \cdot \frac{T^{3}}{3} = \frac{1}{6} k T^{3} $$  
Solve for $k$:  
$$ k = \frac{6D}{T^{3}} $$  

Substitute $k$ into the average acceleration expression:  
$$ a_{\text{avg}} = \frac{1}{2} \left( \frac{6D}{T^{3}} \right) T = \frac{6D}{2 T^{2}} = \frac{3D}{T^{2}} $$
...
**Topic**: work done by expanding gases (1)

**Thumbnail**: An ideal gas undergoes adiabatic expansion with γ=1.4 from 400 K to 300 K at initial volume 0.2 m³; find work done.

### Problem

An ideal gas with adiabatic index $\gamma$ undergoes an adiabatic expansion. The gas starts at an initial volume $V_1$ and initial temperature $T_1$, and expands until its temperature is $T_2$. The gas consists of $n$ moles. Find the work done by the gas.

Express your answer in terms of $n$, $R$ (the universal gas constant), $\gamma$, $T_1$, and $T_2$.

### Solution

The work done by an ideal gas during an adiabatic expansion is given by:  
$$ W = \int_{V_1}^{V_2} P  dV $$  
For an adiabatic process, $PV^\gamma = \text{constant}$. Using the initial state:  
$$ P_1 V_1^\gamma = P V^\gamma \implies P = P_1 V_1^\gamma V^{-\gamma} $$  
Substitute into the work integral:  
$$ W = \int_{V_1}^{V_2} P_1 V_1^\gamma V^{-\gamma}  dV = P_1 V_1^\gamma \int_{V_1}^{V_2} V^{-\gamma}  dV $$  
The integral evaluates to:  
$$ \int V^{-\gamma}  dV = \frac{V^{1-\gamma}}{1-\gamma} $$  
Thus:  
...
Factor $V_1^{1-\gamma}$:  
$$ W = \frac{P_1 V_1^\gamma}{1-\gamma} V_1^{1-\gamma} \left( \left(\frac{V_2}{V_1}\right)^{1-\gamma} - 1 \right) = \frac{P_1 V_1}{1-\gamma} \left( \left(\frac{V_2}{V_1}\right)^{1-\gamma} - 1 \right) $$  
For adiabatic processes, $TV^{\gamma-1} = \text{constant}$:  
$$ T_1 V_1^{\gamma-1} = T_2 V_2^{\gamma-1} \implies \frac{V_2}{V_1} = \left( \frac{T_1}{T_2} \right)^{\frac{1}{\gamma-1}} $$  
...
Thus, the kinetic energy of the car relative to the ground is $\frac{1}{2} m (v_T^2 + v_C^2)$.
...
A portable electric heater with power output $P$ is used to melt a block of ice initially at $0^\circ$C. The ice has mass $m$, and the latent heat of fusion for ice is $L_f$. 
...
The current enclosed by this loop is the entire current $I$ in the wire. Applying Ampère's law:  
...
### Step 6: Express as a power of 10 and round the exponent  
...
**Step 3: Calculate the magnification of the eyepiece ($ m_e $)**  
...
**Step 4: Compute total displacement components**  
...
Given that the current activity is a fraction $ f $ of the initial activity:  
$$ A = f A_0 $$  
...
**Step 5: Solve for $T_{2,\mathrm{C}}$**  
...
The work function $\phi$ is defined as the minimum energy required to remove an electron from the metal surface. In the photoelectric effect, this energy corresponds to the energy of a photon at the threshold wavelength $\lambda_0$, where the maximum kinetic energy of emitted electrons is zero.  
...
Since the electron moves perpendicular to both fields, its velocity $\vec{v}$ has no $y$- or $z$-component and lies along the $x$-axis:
...
Apply Newton's second law for translation along the incline (positive direction down the incline):
...
$$ g \sin\theta = a + \frac{2}{5} a = \frac{7}{5} a $$

Solve for $a$:

$$ a = \frac{5}{7} g \sin\theta $$

Thus, the linear acceleration of the center of mass is $\dfrac{5}{7} g \sin\theta$.

### Answer

$$\boxed{\dfrac{5}{7} g \sin\theta}$$
