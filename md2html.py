...
def process_toc_with_native_mistune(md_text):
    ...
    # Generate TOC HTML using native render_toc_ul
    if filtered_toc_items:
        toc_html = '<details class="toc" open><summary>Table of Contents</summary>'
        toc_html += render_toc_ul(filtered_toc_items)
        toc_html += '</details>'
    else:
        toc_html = ''
...
def add_toc_css():
    ...
    """
    return """
/* Table of Contents Styling - for mistune's <details class="toc"> */
details.toc {
...
def convert_md_to_html(md_file_path, css_file_path='style.css', prism_theme='prism', line_numbers=False, collapse_lines=10, inline_lang='python', enable_toc=True, foldable_sections=None):
    ...
    # Convert markdown to HTML using native mistune TOC functions
    if enable_toc:
